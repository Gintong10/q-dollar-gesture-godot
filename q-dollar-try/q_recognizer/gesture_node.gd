...
var points_raw : Array[Vector2] #gesture points not normalized
...
func recognize_gesture():
	var start_time = Time.get_unix_time_from_system() #captures the initial unix time

	#register_gesture()
	print(points_raw)
	gesture_classified.emit(Recognizer.recognize(points_raw))
	#reset_gesture()
	
	var end_time = Time.get_unix_time_from_system() #captures the end time in unix time
	var elapsed_time = (end_time - start_time) / 60
	print(elapsed_time) 
...
#func register_gesture():
		...
		#for i in range(children.size()):
			#line_to_vec2_array(children[i])
		#normalize_points()
		#save_gesture_to_resource()

func line_to_vec2_array(line : Line2D):
	for point in line.points:
		var vec2 : Vector2 = Vector2(point.x, point.y)
		points_raw.append(vec2)
